...
with st.sidebar.expander("
 Parâmetros do Semáforo", expanded=True):
    ...
    do_warn_low_nitr = st.number_input("Nitrificação – abaixo disso é VERMELHO", value=1.0, 
step=0.1)
    do_warn_high_nitr = st.number_input("Nitrificação – acima disso é VERMELHO", value=4.0, 
step=0.1)
    ...
    ph_warn_low_general = st.number_input("pH Geral – abaixo disso é VERMELHO", value=6.0, 
step=0.1)
    ph_warn_high_general = st.number_input("pH Geral – acima disso é VERMELHO", value=9.0, 
step=0.1)
...
def render_cacambas_gauges(title, n_cols=4):
    # Filtro rígido: só caçambas vão para gauge
    def is_cacamba(label: str):
        base = _strip_accents(label.lower())
        return ("cacamba" in base)
    def is_excluded(label: str):
        base = _strip_accents(label.lower())
        grupos = KW_PH + KW_VAZAO + KW_SST + KW_DQO + KW_OXIG + KW_VALVULA + KW_SOPRADOR + 
KW_NIVEIS_OUTROS
        return any(k in base for k in grupos)
    cols_orig = [c for c in df.columns if is_cacamba(c) and not is_excluded(c)]
...
def cc_desenhar_carta(x, y, titulo, ylabel, mostrar_rotulos=True):
    ...
    if mostrar_rotulos and not y_nonnull.empty:
        ...
        bbox = dict(boxstyle="round,pad=0.25", fc="white", ec="none", alpha=0.7) if cc_lbl_bbox
 else None
...
def cc_guess_item_label(df_txt: pd.DataFrame, header_row: int, col_idx: int, fallback: str) -> 
str:
    label = ""
    if header_row - 1 >= 0:
        try:
...
with st.status("Carregando dados das cartas de controle...", expanded=True) as status:
    try:
        ...
        if cc_hdr is None:
            st.error("
 Não foi possível localizar a linha de cabeçalho (DATA + CUSTO/GASTO).")
            st.stop()
        ...
        status.update(label="Dados carregados com sucesso 
    except requests.exceptions.Timeout:
        st.error("
 Timeout ao acessar o Google Sheets.")
        st.stop()
    except requests.exceptions.RequestException as e:
        st.error(f"
 Falha ao baixar CSV: {e}")
        st.stop()
    except Exception as e:
        st.error(f"
 Erro inesperado ao carregar dados: {e}")
        st.stop()
...
if not cc_cost_idx_list:
    st.error("
 Nenhuma coluna de custo/gasto encontrada.")
...
for aba, item in zip(abas, cc_items):
    with aba:
        ...
        c1.metric("Custo do dia", f"R$ {dia:,.2f}".replace(",", "X").replace(".", 
",").replace("X", "."))
        c2.metric("Custo da semana", f"R$ {semana:,.2f}".replace(",", "X").replace(".", 
",").replace("X", "."))
        c3.metric("Custo do mês", f"R$ {mes:,.2f}".replace(",", "X").replace(".", 
",").replace("X", "."))
        # Agregações
...
st.caption("Selecione e copie o texto acima (Ctrl+C / Cmd+C) para colar no 
WhatsApp/relatório.")
# -----------------------------------------------------------
...
with st.expander("
    st.write("
 Status geral do sistema", expanded=False):
 Aba operacional carregada:", df.shape, "linhas/colunas")
# Pequeno aviso sobre valores ausentes
if df.isna().sum().sum() > 0:
    st.warning("
 Existem valores ausentes em algumas colunas da aba operacional.")
# -----------------------------------------------------------
# RODAPÉ
# -----------------------------------------------------------
st.markdown("---")
...
Se precisar ajustar novas faixas, adicionar sensores ou criar novas métricas, estou aqui para 
ajudar.
